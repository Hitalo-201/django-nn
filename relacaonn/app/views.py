...
def deletar_marca(request,pk ,template_name="marca/marca_delete.html"):
...
def listar_produto_marca(request, pk, template_name="marca/marca_produto_list.html"):
...
def deletar_produto(request, pk, template_name='produto/produto_delete.html'):
...
def listar_categoria(request, template_name='categoria/categoria_list.html'):
    ...
    return render(request, template_name, categorias)
...
def deletar_categoria(request, pk, template_name='categoria/categoria_delete.html'):
    categoria = Categoria.objects.get(pk = pk)
    if request.method=='POST':
        categoria.delete()
        return redirect('listar_categoria')
    return render(request, template_name, {'produto':categoria})

def editar_categoria(request, pk, template_name='produto/produto_form.html'):
    categoria = get_object_or_404(Categoria, pk=pk)
    if request.method == "POST":
        form = ProdutoForm(request.POST, instance=categoria)
        if form.is_valid():
            form.save()
            return redirect('listar_categoria')
    else:
        form = MarcaForm(instance=categoria)
    return render(request, template_name, {'form':form})
    

def listar_produtos_categoria(request, pk, template_name="categoria/categoria_produto_list.html"):
